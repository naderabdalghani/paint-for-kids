...
PlayWithColor::PlayWithColor(ApplicationManager * pApp) :Action(pApp)
{
	Correct = 0;
	Wrong = 0;
}

bool PlayWithColor::ReadActionParameters() {
	return true;
}

//Execute the action
void PlayWithColor::Execute()
{
	Input* pIn = pManager->GetInput();
	Output* pOut = pManager->GetOutput();
	
	int Count = 0;
	PlayingColor = pManager->RandClr(Count);
	
	if (PlayingColor != Empty)
	{
		pOut->PrintMessage("Select all " + PlayingColor.Color2String() + " Figures");
